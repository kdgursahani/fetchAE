...
def get_inactive_users_with_recent_receipts_query():
    # This query summarizes inactive users who have scanned receipts in the last 30 days.
    return '''
        SELECT 
            u._id AS user_id,
...
def get_users_with_no_receipts_query():
    # This query counts users who have never scanned a receipt.
    return '''
        SELECT 
...
def get_users_who_have_never_logged_in_query():
    # This query counts users who have never logged into the app.
    return '''
        SELECT 
...
def get_receipts_with_invalid_user_ids_query():
    # Checks for receipts with invalid userId values (non-existent users).
    return '''
        SELECT 
            COUNT(*) AS invalid_user_id_count
        FROM receipts r
        WHERE r.userId NOT IN (SELECT _id FROM users);
    '''
...
if __name__ == "__main__":
    ...
    'Orphaned receipts': get_receipts_with_invalid_user_ids_query()
    }
